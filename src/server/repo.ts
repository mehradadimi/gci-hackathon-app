...
export async function getDeliveredSeriesForCompany(companyId: number, limit: number = 4): Promise<Array<{ fy: number | null; fp: string | null; label: string; delivered: number }>> {
  const db = getDb();
  const rows = await db.allAsync<{
    fy: number | null;
    fp: string | null;
    ...
    `SELECT p.fy, p.fp, a.actual_value
     FROM periods p
     JOIN actuals a ON a.period_id = p.id AND a.metric = 'revenue'
     WHERE p.company_id = ?
     ORDER BY COALESCE(p.fy, 0) DESC, CASE UPPER(COALESCE(p.fp, '')) WHEN 'FY' THEN 5 WHEN 'Q4' THEN 4 WHEN 'Q3' THEN 3 WHEN 'Q2' THEN 2 WHEN 'Q1' THEN 1 ELSE 0 END DESC
     LIMIT ?`,
    [companyId, limit]
  ...
  return rows
    ...
    .map((r) => ({ fy: r.fy, fp: r.fp, label: `${r.fy ?? ''}${r.fp ? ' ' + r.fp : ''}`.trim(), delivered: Number(r.actual_value) }));
}
